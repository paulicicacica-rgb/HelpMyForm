...
def inject(rel_path, group_langs, self_path):
    ...
    # Strip existing alternate/canonical link tags
    html = re.sub(r'\s*<link[^>]+rel=["\'](?:alternate|canonical)["\'][^>]*/?\s*>', '', html)
    html = re.sub(r'\s*<link[^>]+(?:hreflang|canonical)[^>]*/?\s*>', '', html)

    # Build new tags
    tags = []
    for lang, path in group_langs.items():
        tags.append(f'  <link rel="alternate" hreflang="{lang}" href="{to_url(path)}"/>')
    default_path = group_langs.get("en", list(group_langs.values())[0])
    tags.append(f'  <link rel="alternate" hreflang="x-default" href="{to_url(default_path)}"/>')
    tags.append(f'  <link rel="canonical" href="{to_url(self_path)}"/>')
